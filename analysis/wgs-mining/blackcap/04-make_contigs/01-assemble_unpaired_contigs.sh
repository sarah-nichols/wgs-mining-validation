...
#SBATCH --job-name=assemble_contigs_unpaired
#SBATCH --nodes=1
...
for file in "$NONHOST_UNPAIRED_FASTAS"/unpaired_*.fasta; do
    filename=${file##/*/}
    output="$OUTPUT_MEGA_UNPAIRED/${filename/%.fasta/_assembled}"
    megahit -r "$file" -t 16 -o "$output"
    
done

for subdir in "$OUTPUT_MEGA_UNPAIRED"/*; do
