...
BAM=$(sed -n "${SLURM_ARRAY_TASK_ID}p" "$SAMPLE_IDS_SILVEREYE")

BAM_OUT="$FILTERED_SILVEREYE/${BAM##/*/}"
echo "Copying to $BAM_OUT"
cp "$BAM" "$BAM_OUT"
echo "Editing header with samtools"
BAM_HEADER="$FILTERED_SILVEREYE/header_${BAM##/*/}"
samtools view -H "$BAM_OUT" | sed 's/,//' | samtools reheader - "$BAM_OUT" > "$BAM_HEADER"
rm "$BAM_OUT"
BAM_FILTERED_PAIRED="$FILTERED_SILVEREYE/paired_${BAM##/*/}"
BAM_FILTERED_UNPAIRED="$FILTERED_SILVEREYE/unpaired_${BAM##/*/}"
...
--kmer-file "$HOST_HSS_SILVEREYE" \
--is-host-aligned TRUE \
--filter-bwa-image "$HOST_IMG_SILVEREYE" \
--filter-metrics "$FILTER_METRICS_SILVEREYE/metrics_${BAM##/*/}" \
