...
#SBATCH --job-name=kraken_assign_unpaired_contigs
#SBATCH --nodes=1
...
fasta_files=($OUTPUT_MEGA_UNPAIRED/*.fa)
...
kraken2 --db "$KRAKEN_CAPPED_CUSTOM" \
    ...
    --classified-out \
    --use-names \
    --output "$KRAKEN_UNPAIRED_CONTIGS_MEGA_STATS"/"$sample".stats.report \
    --report "$KRAKEN_UNPAIRED_CONTIGS_MEGA_REPORT"/"$sample".report \
    "$fasta_file"
