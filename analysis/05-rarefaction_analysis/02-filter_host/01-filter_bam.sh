...
# Usage:
#   sbatch --array=1-N this_script.sh <parameters_file> [--mail-user=email@example.com]
#
# Arguments:
...
#samtools reheader -P <(samtools view -H "$BAM_OUT" | sed 's/,//') "$BAM_OUT" > "$BAM_OUT.temp"
#mv "$BAM_OUT.temp" "$BAM_OUT"
